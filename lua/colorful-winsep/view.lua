...
function M.render_left(opts, only_2wins)
    ...
    sep.start_symbol = opts.border[2]
    sep.body_symbol = opts.border[2]
    sep.end_symbol = opts.border[2]
    ...
    if only_2wins then
        ...
        if opts.indicator.position == "center" then
            sep.start_symbol = opts.indicator.symbols.start_left
        elseif opts.indicator.position == "start" then
            sep.start_symbol = opts.indicator.symbols.start_left
        elseif opts.indicator.position == "end" then
            sep.end_symbol = opts.indicator.symbols.end_left
        elseif opts.indicator.position == "both" then
            sep.start_symbol = opts.indicator.symbols.start_left
            sep.end_symbol = opts.indicator.symbols.end_left
        end
    end
...
---@param only_2wins boolean we should deal with 2 windows situation
function M.render_down(opts, only_2wins)
    ...
    sep.start_symbol = opts.border[1]
    sep.body_symbol = opts.border[1]
    sep.end_symbol = opts.border[1]
    ...
    if only_2wins then
        ...
        if opts.indicator.position == "center" then
            sep.end_symbol = opts.indicator.symbols.end_down
        elseif opts.indicator.position == "start" then
            sep.start_symbol = opts.indicator.symbols.start_down
        elseif opts.indicator.position == "end" then
            sep.end_symbol = opts.indicator.symbols.end_down
        elseif opts.indicator.position == "both" then
            sep.start_symbol = opts.indicator.symbols.start_down
...
---@param only_2wins boolean we should deal with 2 windows situation
function M.render_up(opts, only_2wins)
    local sep_width = fn.winwidth(0)
    local current_row, current_col = unpack(api.nvim_win_get_position(0))
    ...
    if only_2wins then
        ...
        sep_width = math.ceil(sep_width / 2)
        if opts.indicator.position == "center" then
            sep.start_symbol = opts.indicator.symbols.start_up
        elseif opts.indicator.position == "start" then
            sep.start_symbol = opts.indicator.symbols.start_up
        elseif opts.indicator.position == "end" then
            sep.end_symbol = opts.indicator.symbols.end_up
        elseif opts.indicator.position == "both" then
            sep.start_symbol = opts.indicator.symbols.start_up
            sep.end_symbol = opts.indicator.symbols.end_up
        end
...
---@param only_2wins boolean we should deal with 2 windows situation
function M.render_right(opts, only_2wins)
    local sep_height = fn.winheight(0)
    local current_row, current_col = unpack(api.nvim_win_get_position(0))
    local anchor_row = current_row
    ...
    if only_2wins then
        sep_height = math.ceil(sep_height / 2)
        if opts.indicator.position == "center" then
            sep.end_symbol = opts.indicator.symbols.end_right
        elseif opts.indicator.position == "start" then
            sep.start_symbol = opts.indicator.symbols.start_right
        elseif opts.indicator.position == "end" then
            sep.end_symbol = opts.indicator.symbols.end_right
        elseif opts.indicator.position == "both" then
            sep.start_symbol = opts.indicator.symbols.start_right
...
function M.render(opts)
    ...
    if utils.has_adjacent_win(directions.left) then
        M.render_left(opts, only_2wins)
    else
        M.separators.left:hide()
    end
    if utils.has_adjacent_win(directions.down) then
        M.render_down(opts, only_2wins)
    else
        M.separators.down:hide()
    end
    if utils.has_adjacent_win(directions.up) then
        M.render_up(opts, only_2wins)
    else
        M.separators.up:hide()
    end
    if utils.has_adjacent_win(directions.right) then
        M.render_right(opts, only_2wins)
    else
